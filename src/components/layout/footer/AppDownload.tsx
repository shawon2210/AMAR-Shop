    ...
    <div className="flex flex-col sm:flex-row lg:flex-col gap-4 justify-center lg:justify-start">
      {stores.map((store) => (
        <Link
          key={store.label}
          href={store.href}
          className="inline-flex items-center justify-center gap-3 w-full sm:w-auto lg:w-full max-w-[210px] mx-auto lg:mx-0 h-[60px] rounded-[14px] border border-gray-200 bg-white hover:bg-[#F8FAFC] hover:border-gray-300 transition-all duration-200 text-gray-700"
        >
          <span className="w-5 h-5 flex items-center justify-center shrink-0">{store.svg}</span>
          <div className="text-left">
            <p className="text-[10px] text-gray-400 leading-none mb-0.5">Download on</p>
            <p className="text-sm font-semibold leading-none">{store.label}</p>
          </div>
        </Link>
      ))}
    </div>
