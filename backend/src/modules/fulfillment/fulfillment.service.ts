...
export class FulfillmentService {
  ...
  async assignWarehouse(orderId: string) {
    ...
    for (const wh of warehouses) {
      let score = 0;
      const inv = await this.prisma.inventory.findMany({ where: { warehouseId: wh.id } });
      ...
      if (score < bestScore) { bestScore = score; bestWarehouse = wh; }
    }
  ...
  async selectCourier(orderId: string) {
    ...
    const couriers = await this.prisma.courier.findMany({ where: { isActive: true }, include: { zones: true } });
    ...
    for (const c of couriers) {
      const zone = c.zones.find(z => z.isActive && z.districts.includes(district));
      const baseFee = zone?.baseFee || c.baseRate;
      const cost = baseFee + totalWeight * c.perKgRate + (order.paymentMethod === 'COD' ? (zone?.codFee || c.codFee) : 0);

      if (cost < bestCost) { bestCost = cost; best = { courier: c, cost, zone, estimatedDays: zone?.estDays || c.deliveryDays }; }
    }
  ...
  async createShipment(orderId: string) {
    ...
    const shipment = await this.prisma.shipment.create({
      data: {
        ...
        trackingId,
        status: 'PENDING',
        weight: order.items.reduce((s, i) => s + (i.product.weight || 0.5) * i.quantity, 0),
    ...
    await this.prisma.shipmentTimeline.create({
      data: { shipmentId: shipment.id, status: 'PENDING', note: 'Shipment created' },
    });

    await this.prisma.order.update({ where: { id: orderId }, data: { trackingNumber: trackingId, status: 'PROCESSING' } });
  ...
  async getDeliverySlots(pincode: string) {
    ...
    for (const c of couriers) {
      ...
      const availableSlots = slotsForCourier.filter(s => s.bookedOrders < s.maxOrders);
      for (const s of availableSlots) {
        slots.push({ courier: c.name, slotId: s.id, dayOfWeek: s.dayOfWeek, startTime: s.startTime, endTime: s.endTime, available: s.maxOrders - s.bookedOrders });
      }
    }
    return slots;
  }

  async schedulePickup(shipmentId: string, slot: { date: string; timeSlot: string }) {
    const shipment = await this.prisma.shipment.findUnique({ where: { id: shipmentId } });
    if (!shipment) throw new NotFoundException('Shipment not found');

    await this.prisma.shipment.update({
      where: { id: shipmentId },
      data: { status: 'PICKUP_SCHEDULED', notes: `Pickup scheduled: ${slot.date} ${slot.timeSlot}` },
    });

    await this.prisma.shipmentTimeline.create({
      data: { shipmentId, status: 'PICKUP_SCHEDULED', note: `Pickup scheduled for ${slot.date} at ${slot.timeSlot}` },
    });
  ...
  async calculateSLA(fromWarehouse: string, toAddress: string) {
    ...
    const couriers = await this.prisma.courier.findMany({
      where: { isActive: true },
      include: { zones: { where: { districts: { has: district || city }, isActive: true } } },
    });
  ...
  async getFulfillmentOptions(sellerId: string) {
    const seller = await this.prisma.store.findUnique({ where: { id: sellerId } });
    if (!seller) throw new NotFoundException('Seller not found');

    const warehouses = await this.prisma.warehouse.count({ where: { isActive: true } });
    const couriers = await this.prisma.courier.count({ where: { isActive: true } });

    return {
      fbl: { available: warehouses > 0, warehouses, description: 'Fulfillment by AmrShop' },
  ...
  async processCODReconciliation(date: string) {
    ...
    return {
      ...
      shipments: shipments.map(s => ({ id: s.id, trackingId: s.trackingId, codAmount: s.codAmount, fee: s.shippingFee })),
    };
  }

  async getCourierPerformance(dateRange: { start: string; end: string }) {
    const logs = await this.prisma.courierPerformanceLog.findMany({
      where: { date: { gte: new Date(dateRange.start), lte: new Date(dateRange.end) } },
    ...
    for (const log of logs) {
      if (!aggregated[log.courierId]) {
        aggregated[log.courierId] = { courierId: log.courierId, courierName: log.courier.name, totalShipments: 0, delivered: 0, failed: 0, returned: 0, onTime: 0, codCollected: 0, codRemitted: 0 };
      }
    ...
    return Object.values(aggregated).map((a: any) => ({
      ...a,
      deliveryRate: a.totalShipments > 0 ? (a.delivered / a.totalShipments * 100).toFixed(1) : 0,
      onTimeRate: a.delivered > 0 ? (a.onTime / a.delivered * 100).toFixed(1) : 0,
      failureRate: a.totalShipments > 0 ? (a.failed / a.totalShipments * 100).toFixed(1) : 0,
    }));
  }
  ...
  private async parseAddress(address: string) {
    ...
    return { street: parts[0] || '', city: parts[1] || '', district: parts[2] || '', division: parts[3] || '' };
  }
}
