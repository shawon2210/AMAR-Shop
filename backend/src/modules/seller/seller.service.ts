...
export class SellerService {
  ...
  async getDashboard(sellerId: string) {
    ...
    const [totalProducts, orderStats, recentOrders, revenueData, sellerProfile] = await Promise.all([
  ...
  async getProducts(
    sellerId: string,
    query: { page?: number; limit?: number; status?: string; search?: string; category?: string },
  ) {
    const store = await this.getStore(sellerId);
  ...
  async updateOrderStatus(orderItemId: string, sellerId: string, status: string) {
    const store = await this.getStore(sellerId);
    ...
    if (orderItem.product.storeId !== store.id) throw new ForbiddenException('Not your product');
  ...
  async getAnalytics(
    ...
    const [salesData, topProducts, categoryBreakdown, totalViews] = await Promise.all([
      this.prisma.orderItem.findMany({
        where: {
          product: { storeId: store.id },
      ...
      this.prisma.product.findMany({
        ...
        take: 10,
        select: { id: true, name: true, slug: true, soldCount: true, price: true, stockCount: true, images: true },
      }),
      ...
      this.prisma.browsingHistory.count({
        where: { product: { storeId: store.id }, createdAt: { gte: from, lte: to } },
      }),
    ]);
    ...
    for (let i = 0; i <= Math.ceil((to.getTime() - from.getTime()) / (24 * 60 * 60 * 1000)); i++) {
    ...
    const conversion = totalViews > 0 ? Number(((totalOrders / totalViews) * 100).toFixed(2)) : 0;

    return { salesChart, topProducts, categoryBreakdown: categoriesWithNames, totalViews, totalOrders, conversion };
  }

  async getFinance(sellerId: string) {
    const store = await this.getStore(sellerId);
    const wallet = await this.prisma.wallet.findUnique({ where: { userId: sellerId } });
    ...
    return {
      wallet: wallet ? { balance: wallet.balance, totalEarned: wallet.totalEarned, totalSpent: wallet.totalSpent } : { balance: 0, totalEarned: 0, totalSpent: 0 },
  ...
  async updateStore(sellerId: string, data: any) {
    ...
    if (Object.keys(storeData).length > 0) {
      await this.prisma.store.update({ where: { id: store.id }, data: storeData });
    }

    if (data.vacationMode !== undefined || data.vacationMessage !== undefined || data.vacationUntil !== undefined) {
      const profileData: Record<string, unknown> = {};
      if (data.vacationMode !== undefined) profileData.onVacation = data.vacationMode;
      if (data.vacationMessage !== undefined) profileData.vacationMessage = data.vacationMessage;
      if (data.vacationUntil !== undefined) profileData.vacationUntil = new Date(data.vacationUntil);
  ...
  async getKycStatus(userId: string) {
    ...
    if (!profile) {
      return { isKycVerified: false, kycSubmitted: false, message: 'No KYC data found. Please complete your seller profile.' };
    }
