...
export class BullQueueService implements OnModuleDestroy {
  ...
  constructor(private prisma: PrismaService) {
    ...
    this.orderQueue = new Queue<OrderJobData>('order-processing', { connection, defaultJobOptions: realtimeConfig.bull.defaultJobOptions });
    this.notificationQueue = new Queue<NotificationJobData>('notification', { connection, defaultJobOptions: realtimeConfig.bull.defaultJobOptions });
    this.emailQueue = new Queue<EmailJobData>('email', { connection, defaultJobOptions: realtimeConfig.bull.defaultJobOptions });
    this.smsQueue = new Queue<any>('sms', { connection, defaultJobOptions: realtimeConfig.bull.defaultJobOptions });
    this.inventorySyncQueue = new Queue<InventorySyncJobData>('inventory-sync', { connection, defaultJobOptions: realtimeConfig.bull.defaultJobOptions });
    this.searchIndexQueue = new Queue<SearchIndexJobData>('search-index', { connection, defaultJobOptions: realtimeConfig.bull.defaultJobOptions });
    this.analyticsQueue = new Queue<AnalyticsJobData>('analytics', { connection, defaultJobOptions: realtimeConfig.bull.defaultJobOptions });
    this.payoutQueue = new Queue<PayoutJobData>('payout', { connection, defaultJobOptions: realtimeConfig.bull.defaultJobOptions });

    this.orderQueueEvents = new QueueEvents('order-processing', { connection });
    this.notificationQueueEvents = new QueueEvents('notification', { connection });
  ...
  async addNotificationJob(data: NotificationJobData, delay = 0): Promise<Job<NotificationJobData>> {
  ...
  async addAnalyticsJob(data: AnalyticsJobData, delay = 0): Promise<Job<AnalyticsJobData>> {
    return this.analyticsQueue.add(data.event, data, { delay });
  }

  async addInventorySyncJob(data: InventorySyncJobData): Promise<Job<InventorySyncJobData>> {
    return this.inventorySyncQueue.add(data.type, data);
  }

  async addSearchIndexJob(data: SearchIndexJobData): Promise<Job<SearchIndexJobData>> {
  ...
  private registerConsumers(): void {
    ...
    const orderWorker = new Worker<OrderJobData>('order-processing', async job => {
      this.logger.log(`Processing order job: ${job.id} - ${job.data.action} for order ${job.data.orderId}`);
    ...
    const inventoryWorker = new Worker<InventorySyncJobData>('inventory-sync', async job => {
      this.logger.log(`Syncing inventory: ${job.data.productId} - ${job.data.type}`);
  ...
  private async handleInventorySync(data: InventorySyncJobData): Promise<void> {
    ...
    switch (data.type) {
      case 'add': newQuantity += data.quantity; break;
      case 'remove': newQuantity = Math.max(0, newQuantity - data.quantity); break;
      case 'reserve': newQuantity = Math.max(0, newQuantity - data.quantity); break;
      case 'release': newQuantity += data.quantity; break;
    }
