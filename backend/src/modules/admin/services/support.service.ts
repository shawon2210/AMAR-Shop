...
export class AdminSupportService {
  ...
  async getSupportTicket(id: string) {
    const ticket = await this.prisma.supportTicket.findUnique({
      ...
      include: {
        ...
        messages: { orderBy: { createdAt: 'asc' }, include: { sender: { select: { id: true, name: true, avatar: true } } } },
      },
    });
  ...
  async replyToTicket(id: string, senderId: string, content: string) {
    ...
    const [message] = await Promise.all([
      ...
      this.prisma.supportTicket.update({ where: { id }, data: { status: 'IN_PROGRESS' as const } }),
    ]);
    return { ...message, user: message.sender };
  }

  async updateSupportTicket(id: string, data: { status?: string; priority?: string; assignedTo?: string }) {
    const ticket = await this.prisma.supportTicket.findUnique({ where: { id } });
    if (!ticket) throw new NotFoundException('Support ticket not found');
  ...
  async updateReview(reviewId: string, data: { status?: string; reported?: boolean }) {
