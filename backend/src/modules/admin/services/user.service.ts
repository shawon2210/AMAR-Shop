...
export class AdminUserService {
    ...
    const [users, total] = await Promise.all([
      this.prisma.user.findMany({
        ...
        select: {
          ...
          isSeller: true, isActive: true, isVerified: true, createdAt: true,
  ...
  async createAdminUser(data: { name: string; phone: string; password: string; email?: string }) {
  ...
  async getAdminCreators(query: { page?: number; limit?: number }) {
    ...
    const condition = { OR: [{ reviews: { some: {} } }, { storeFollowers: { some: {} } }] };
